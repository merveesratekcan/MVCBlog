...
namespace HS14MVC.Applicationn.Services.ArticleServices
{
    public class ArticleService : IArticleService
    {
        ...
        public async Task<IDataResult<ArticleDTO>> UpdateAsync(ArticleUpdateDTO articleUpdateDTO)
        {
            ...
            if (article is null)
            {
                return new ErrorDataResult<ArticleDTO>(article.Adapt<ArticleDTO>(), "Makale Bulunamadı");
            }
            var updatedArticle = articleUpdateDTO.Adapt(article);
            await _articleRepository.UpdateAsync(updatedArticle);
            await _articleRepository.SaveChangesAsync();
            return new SuccessDataResult<ArticleDTO>(updatedArticle.Adapt<ArticleDTO>(), "Makale Güncelleme Başarılı");
        }
